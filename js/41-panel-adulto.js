...
CB.adulto.pintar = function () {
  ...
  CB.gen.problemas.SUBTIPOS.forEach(function (s) {
    ...
    var td1 = CB.ui.crear('td', 'dato', String(p.intentos)); tr.appendChild(td1);
    var td2 = CB.ui.crear('td', 'dato',
      p.aciertos + ' (' + Math.round(p.aciertos / p.intentos * 100) + ' %)');
    tr.appendChild(td2);
    tr.appendChild(CB.ui.crear('td', 'dato', Math.round(p.rtMedioMs / 1000) + ' s'));
    t2.appendChild(tr);
  });
...
CB.adulto.metrica = function (cont, etiqueta, valor) {
  ...
  d.appendChild(CB.ui.crear('span', 'valor', valor));
  cont.appendChild(d);
};
...
CB.adulto.fichaRefuerzo = function (perfil, codigoError) {
  ...
  for (i = 0; i < 10 && nivel; i++) {
    ...
    if (!item) continue;
    rej.appendChild(CB.ui.crear('div', 'ejercicio',
      (item.consigna || item.enunciado || '') + '  ='));
  }
  ficha.appendChild(rej);
  ficha.appendChild(CB.ui.crear('p', 'pie-ficha',
