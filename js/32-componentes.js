...
CB.componentes.montar = function (contenedor, bloqueoMs, alDesbloquear) {
  ...
  setTimeout(function () {
    for (i = 0; i < botones.length; i++) {
      ...
      botones[i].classList.remove('monta');
      botones[i].classList.add('destello');
    }
    setTimeout(function () {
      for (i = 0; i < botones.length; i++) botones[i].classList.remove('destello');
    }, 140);
...
CB.componentes.conectarToc = function (contenedor) {
  contenedor.addEventListener('pointerdown', function (ev) {
    ...
    if (b && b.classList && b.classList.contains('btn-bloque')) {
      b.classList.add('toc');
      setTimeout(function () { b.classList.remove('toc'); }, 260);
    }
    CB.audio.sfx('toc');
    ev.preventDefault();
  });
...
CB.componentes.pedirConfirmacion = function (boton, alConfirmar) {
  ...
  boton.classList.add('esta-hundido');
  setTimeout(function () { boton.classList.remove('esta-hundido'); }, 300);
