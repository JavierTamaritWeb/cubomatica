...
CB.pruebas.suite('Tipografía: cobertura de glifos y rama activa de la cascada', function () {
  const CRITICOS = 'ÁÉÍÓÚÜÑáéíóúüñ¿¡«»€0123456789';
  const t = CB.pruebas;
  ...
  const lectura = getComputedStyle(document.documentElement)
  ...
  const reglaEnunciado = t.claseEnHoja('.enunciado');
  ...
  const enun = document.querySelector('.enunciado') || document.createElement('div');
  enun.className = 'enunciado';
  document.body.appendChild(enun);
  const ff = getComputedStyle(enun).fontFamily;
