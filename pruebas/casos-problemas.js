...
CB.pruebas.suite('Problemas: 20 estructuras, validador e invariantes 7, 8 y 10', function () {
  const t = CB.pruebas;
  const SUB = CB.gen.problemas.SUBTIPOS;
  ...
  const porSubtipo = CB.pruebas.modoLargo ? 400 : 60;
  let invalidos = 0;
  const motivos = {};
  let primerMalo = null;
  let resolverDistinto = 0, primerResolver = null;
  const unicosPorSubtipo = {};
  let respaldos = 0;
  let i, s, k;
  ...
  for (i = 0; i < SUB.length; i++) {
    ...
    for (k = 0; k < porSubtipo; k++) {
      const rng = CB.util.mulberry32(CB.util.hash32(s + '@' + k));
      const it = CB.gen.problemas.generarSubtipo(s, rng, (k % 3) + 1, {
      ...
      const v = CB.gen.problemas.validar(it);
      if (!v.ok) {
        invalidos++;
      ...
      const indep = CB.gen.problemas.resolver(it.subtipo, it.datos);
  ...
  const pobres = SUB.filter(function (x) { return unicosPorSubtipo[x] < porSubtipo * 0.6; });
  ...
  for (i = 0; i < SUB.length; i++) {
    for (k = 0; k < 40; k++) {
      const it2 = CB.gen.problemas.generarSubtipo(SUB[i],
        CB.util.mulberry32(k * 977 + i), 2,
        { techo: 99, bolsas: CB.gen.problemas.nuevoEstadoBolsas() });
      const f = it2.frases;
      const ultima = f[f.length - 1];
      const lineas = CB.util.cortarLineas(it2.enunciado, 34);
      const anchoOk = lineas.every(function (l) { return l.length <= 34; });
      const mide = CB.ui.medirLineas(it2.enunciado);
      const coincide = mide.length === lineas.length;
  ...
  for (i = 0; i < SUB.length; i++) {
    for (k = 0; k < 40; k++) {
      const it3 = CB.gen.problemas.generarSubtipo(SUB[i],
  ...
  CB.catalogo.CON_DATO_SOBRANTE.forEach(function (nivelId) {
    ...
    for (k = 0; k < 200; k++) {
      const it4 = CB.gen.problemas.generarSubtipo(sub,
      ...
      const nums = (it4.enunciado.match(/\d+/g) || []).length;
  ...
  let conOpciones = 0;
  for (i = 0; i < SUB.length; i++) {
    const it5 = CB.gen.problemas.generarSubtipo(SUB[i], CB.util.mulberry32(i + 1), 2,
      { techo: 99, bolsas: CB.gen.problemas.nuevoEstadoBolsas() });
  ...
  let bolsas = CB.gen.problemas.nuevoEstadoBolsas();
  const rngG = CB.util.mulberry32(20260725);
  let f1 = 0, m1 = 0;
  for (k = 0; k < 200; k++) {
    const act = CB.gen.problemas.elegirActores(rngG, bolsas);
    bolsas = act.bolsas;
  ...
  let bolsas2 = CB.gen.problemas.nuevoEstadoBolsas();
  let f2 = 0;
  for (k = 0; k < 5000; k++) {
    const a2 = CB.gen.problemas.elegirActores(rngG, bolsas2);
    bolsas2 = a2.bolsas;
  ...
  const prohibidos = CB.datos.NOMBRES_F.concat(CB.datos.NOMBRES_M).filter(function (n) {
  ...
  const cuenta = {};
  for (k = 0; k < 40 * 3; k++) {
    const sub2 = CB.gen.problemas.siguienteSubtipo(p2, { lucesActuales: 3 });
    cuenta[sub2] = (cuenta[sub2] || 0) + 1;
    p2.problemas[sub2].intentos++;
    p2.problemas[sub2].aciertos++;
  }
  const nucleares = CB.gen.problemas.NUCLEAR;
  const valores = nucleares.map(function (x) { return cuenta[x] || 0; });
  const maxV = Math.max.apply(null, valores), minV = Math.min.apply(null, valores);
  ...
  const p3 = CB.pruebas.perfilNuevo();
  p3.trimestreDeducido = 3;
  let conPocasLuces = 0;
  for (k = 0; k < 200; k++) {
    const sub3 = CB.gen.problemas.siguienteSubtipo(p3, { lucesActuales: 2 });
