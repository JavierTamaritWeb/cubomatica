...
- **Run one suite**: filter before running.
  ...
  ```
- Results land in `document.getElementById('resumen').textContent`. Current baseline: **704 checks, 0 failures** (deterministic).
...
**Every bug ever fixed has a guard in `pruebas/casos-regresiones.js`.** Its header lists all eighty-four found so far (E1-E84) and where each guard lives. The rule it states: a bug fixed without a test comes back. Add to it before closing any defect.
