...
- **Run one suite**: filter before running.
  ...
  ```
- Results land in `document.getElementById('resumen').textContent`. Current baseline: **612 checks, 0 failures** (deterministic).
...
**Every bug ever fixed has a guard in `pruebas/casos-regresiones.js`.** Its header lists all sixty-eight found so far (E1-E68) and where each guard lives. The rule it states: a bug fixed without a test comes back. Add to it before closing any defect.
